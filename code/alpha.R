...
df_alpha = read.table('/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/outputs/jobs02/metadata_shannon.tsv', 
...
# calculate adiv
for (i in 1:1) {
  stats.table.all[i,1] <- colnames(df_alpha)[i+3]
  stats.table.all[i,2] <- wilcox.test(df_alpha[,i+3] ~ Diagnosis, data = df_alpha, paired = TRUE)$p.value
  stats.table.all[i,3] <- t.test(df_alpha[,i+3] ~ Diagnosis, data = df_alpha, paired = TRUE)$p.value
}
...
for (i in 1:1) {
  ...
  p <- ggplot(data = d.final, aes(x = Diagnosis, y = shannon_entropy, fill = Diagnosis)) +
    ...
    geom_text_repel(data = subset(d.final, Diagnosis == "1_Unaffected"), aes(label = TwinPair), 
                    nudge_x = -0.5, size = 2, color = "black", segment.alpha = 0.3) +
    scale_x_discrete(labels = c("Unaffected", "RA")) +
    scale_fill_manual(values = col1) +
    scale_linetype_manual(values = line1, name = "Shannon Entropy Difference", labels = c("Increase", "Decrease", "No change")) +
    scale_color_manual(values = col2, name = "Shannon Entropy Difference", labels = c("Increase", "Decrease", "No change")) +      
    xlab(NULL) +
