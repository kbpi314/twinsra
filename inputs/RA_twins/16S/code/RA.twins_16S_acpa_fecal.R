...
# Load R scripts
load(file="/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/inputs/RA_twins/16S/code/RA.twins_16S_prep_KB.RData")
load(file="/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/inputs/RA_twins/16S/code/RA.twins_16S_acpa_fecal_KB.RData")

# Save R script
# Do this step prior to closing R
save.image(file="/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/inputs/RA_twins/16S/code/RA.twins_16S_acpa_fecal_KB.RData")
...
### ACPA fecal statistics ###

# directory
dir = "/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/inputs/RA_twins/16S/jobs/10_acpa_fecal_KB/"

# read in data table
d <- read.table(file = paste(dir, "acpa_fecal.txt", sep = ""),
...
stats.table.all <- matrix(data = NA, nrow = 116, ncol = 3)
colnames(stats.table.all) <- c("acpa fecal analyte", "wilcoxon", "ttest")

stats.table.clean <- matrix(data = NA, nrow = 116, ncol = 3)
colnames(stats.table.clean) <- c("acpa fecal analyte", "wilcoxon", "ttest")

# calculate wilcoxon and ttest between Unaffected/RA siblings for each analyte
for (i in 1:116) {
...
# save
ft.clean = paste(dir, "acpa_fecal_stats_clean.csv", sep = "")
...
dir = "/Users/KevinBu/Desktop/clemente_lab/Projects/twinsra/inputs/RA_twins/16S/jobs/10_acpa_fecal_KB/clean/"
...
# for every fecal analyte in the table, calculate the change in abundance between siblings 
for (i in 1:116) {
...
# create dataframes
d.clean <- d[!d$Sibling_pair == "sib_10",] # remove 182, 183

# create tables for storing wilcoxon and ttest results
stats.table.clean <- matrix(data = NA, nrow = 116, ncol = 7)
colnames(stats.table.clean) <- c("acpa plasma analyte", "mean_u", "sd_u", 
                                 "mean_ra", "sd_ra", "wilcoxon", "ttest")

# calculate wilcoxon and ttest between Unaffected/RA siblings for each analyte
for (i in 1:116) {
  stats.table.clean[i,1] <- colnames(d.clean)[i+3]
  stats.table.clean[i,2] <- mean(d.clean[c(FALSE, TRUE),c(i+3)])
  stats.table.clean[i,3] <- sd(d.clean[c(FALSE, TRUE),c(i+3)])
  stats.table.clean[i,4] <- mean(d.clean[c(TRUE, FALSE),c(i+3)])
  stats.table.clean[i,5] <- sd(d.clean[c(TRUE, FALSE),c(i+3)])
  stats.table.clean[i,6] <- wilcox.test(d.clean[,i+3] ~ Diagnosis, data = d.clean, paired = TRUE)$p.value
  stats.table.clean[i,7] <- t.test(d.clean[,i+3] ~ Diagnosis, data = d.clean, paired = TRUE)$p.value
}

# save
ft.clean = paste(dir, "acpa_fecal_stats_clean_v2.csv", sep = "")
write.csv(file = ft.clean, stats.table.clean)
